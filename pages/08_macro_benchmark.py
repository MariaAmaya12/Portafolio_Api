...
col1.metric("Tasa libre de riesgo (%)", f"{macro['risk_free_rate_pct']:.2f}" if macro["risk_free_rate_pct"] == macro["risk_free_rate_pct"] else "N/D")
col2.metric("Inflación interanual", f"{macro['inflation_yoy']:.2%}" if macro["inflation_yoy"] == macro["inflation_yoy"] else "N/D")
col3.metric("COP por USD", f"{macro['cop_per_usd']:.2f}" if macro["cop_per_usd"] == macro["cop_per_usd"] else "N/D")
...
st.plotly_chart(plot_benchmark_base100(cum_port, cum_bench), use_container_width=True)
